        ...
        <% if $Results %>
            <div class="row">
                <table class="table">
                    <thead>
                        <tr>
                            <th style="width: 10%;"><%t Librarian.COVER 'Cover' %></th>
                            <th style="width: 40%;"><%t Librarian.BOOK 'Book' %></th>
                            <th><%t Librarian.PATRON 'Patron' %></th>
                            <th><%t Librarian.LOAN 'Loan' %></th>
                            <% if ReturnAction %><th><%t Librarian.ACTION 'Action' %></th><% end_if %>
                        </tr>
                    </thead>
                    <tbody>
                        <% loop $Results %>
                        <tr>
                            ...
                            <td>
                                <% if $BookName %><b><a href="$ObjectLink">$BookName</a></b><% end_if %>
                                ...
                                <% if $ISBN %><span class="information"><%t Librarian.ISBN 'ISBN' %>: $ISBN</span><% end_if %>
                                <% if $Edition || $PublishYear %><span class="information"><%t Librarian.EDITION 'Edition' %>: $Edition ($PublishYear)</span><% end_if %>
                                <% if $Publisher && $Publisher.Name %>
                                    <span class="information">
                                        <%t Librarian.PUBLISHER 'Publisher' %>: <a href="$Publisher.ObjectLink" title="$Publisher.Name">$Publisher.Title</a>
                                    </span>
                                <% end_if %>
                            ...
                            <td>
                                <%t Librarian.LOAN_DATE 'Loan Date' %>: <p>$LoanDate</p>
                                <%t Librarian.DUE_DATE 'Due Date' %>: <p>$DueDate</p>
                                <% if $ReturnedBy %><%t Librarian.RETURN_DATE 'Return Date' %>: <p>$Created</p><% end_if %>
                            </td>
